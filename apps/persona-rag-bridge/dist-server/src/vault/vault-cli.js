...
program
    ...
    try {
        ...
        await promises_1.default.writeFile(outputFile, envContent);
...
program
    ...
    try {
        ...
        if (!await promises_1.default.access(inputFile).then(() => true).catch(() => false)) {
            console.error(`❌ File not found: ${inputFile}`);
            process.exit(1);
        }
        const content = await promises_1.default.readFile(inputFile, 'utf8');
    ...
    try {
        ...
        if (secrets.length > 0) {
            ...
            const categories = {
                ...
                'Other': secrets.filter(s => !s.includes('API_KEY') &&
                    !s.includes('PASSWORD') &&
                    !s.includes('SECRET') &&
                    !s.includes('TOKEN')).length
            };
