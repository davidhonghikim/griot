...
import type { Job as SDKJob } from 'griot-sdk';
...
export const Jobs: React.FC = () => {
  ...
  const loadJobs = async () => {
    try {
      ...
      // Transform SDK jobs to include display properties
      const transformedJobs = jobsData.map((job: SDKJob) => ({
        jobId: job.jobId,
        id: job.jobId, // Map jobId to id for consistency
        type: (job.metadata as Record<string, unknown>)?.type as string || 'unknown',
        status: job.status,
        parameters: (job.metadata as Record<string, unknown>)?.parameters as Record<string, unknown> || {},
        metadata: job.metadata as Record<string, unknown>,
        createdAt: job.metadata.createdAt,
        updatedAt: job.metadata.completedAt
      }));
                ...
                {selectedTemplate && Object.entries(selectedTemplate.parameters).map(([key, param]) => (
                  <div key={key}>
                    ...
                    {param.type === 'textarea' ? (
                      ...
                      <select
                        className="select-modern"
                        value={String(formData.parameters[key] || param.defaultValue || '')}
                        onChange={(e) => setFormData({
                          ...formData,
                          parameters: { ...formData.parameters, [key]: e.target.value }
                        })}
                      ...
                      <tr key={job.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <div className="text-sm font-medium text-gray-900">{job.type || 'Unknown'}</div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {String(job.metadata?.template || 'Custom')}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
