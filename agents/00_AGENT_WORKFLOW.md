...
- **Handoffs:** The primary handoff file **MUST** be `agents/handoff/LATEST_HANDOFF.yml`. Archived handoffs **MUST** follow the format `[YYYY-MM-DD]_[AgentName]_[Description].md`.
...
### 4. The .bak Method (CRITICAL FOR RISKY EDITS)

**MANDATORY RULE:** For any edit that involves:
- Multi-line changes
- Complex sed/awk operations
- Function additions or modifications
- Type changes
- Any operation that could break the build

You **MUST** follow this process:

1. **Create Backup:** `cp target_file target_file.backup`
2. **Make Changes:** Perform your edits
3. **Test Build:** `npm run build` or equivalent
4. **If Build Fails:** `cp target_file.backup target_file` and try a different approach
5. **If Build Succeeds:** Keep the backup for reference

**Example:**
```bash
# Before making risky changes
cp src/components/ServiceForm.tsx src/components/ServiceForm.tsx.backup

# Make your changes
sed -i '' 's/old_pattern/new_pattern/g' src/components/ServiceForm.tsx

# Test the build
npm run build

# If it fails, restore and try again
cp src/components/ServiceForm.tsx.backup src/components/ServiceForm.tsx
```

**Why This Matters:**
- Prevents irreversible damage to working code
- Allows quick recovery from failed edits
- Maintains build integrity
- Saves time compared to manual fixes
...
2.  Execute the `mv` command:
    ```bash
    mv agents/handoff/LATEST_HANDOFF.yml agents/handoff/archive/[YYYY-MM-DD]_[YourName]_[Description].yml
    ```

**C. Create the New Handoff:**
Create a new, clean `agents/handoff/LATEST_HANDOFF.yml` file. Use the following template:
